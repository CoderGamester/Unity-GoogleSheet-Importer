# GameLovers.GoogleSheetImporter Tests — AI Agent Guide

This file contains testing conventions for the `com.gamelovers.googlesheetimporter` package. It is the source of truth when reading, editing, or creating test files under `Tests/`.
...
single `Tests/Editor/` assembly, one fixture (`CsvParserTest`), and it cannot
reach the `Editor/` assembly (the test asmdef does not reference it) — see the
parent [`AGENTS.md`](../AGENTS.md) for the package's overall architecture.

## 4. Namespace and Suppression

Not yet documented — see `CsvParserTest.cs` for the current (unaudited) convention.
...
but is not currently used anywhere in `Tests/`.
...
Not yet documented. Note: no `[SetUp]`/`[TearDown]` exist in this package's suite
today — the sole fixture is stateless.
...
Not yet documented. No performance fixtures exist today.

## 12. Test Directory Layout

| Directory | Contents |
|---|---|
| `Tests/Editor/` | The package's entire automated suite (`CsvParserTest.cs` only) |

## 13. Coverage Register

**Baseline — runtime assembly: 66.2% (147/222), measured 2026-08-04.**
Editor assembly: **0.0% (0/185)** — near-zero by policy; the ACCEPTED (iii) rows below are why.
...
untested symbol here is accepted; it is simply unreviewed. In particular, the
entire `Editor/` assembly (import window, menu items) is unreachable from the
test asmdef today and is a likely OPEN candidate once audited — do not treat its
absence from this table as an ACCEPTED verdict.
